...
function isInsideCircle(circle, point) {
  ...
  const c = Math.sqrt((point.x - circle.center.x) ** 2 + (point.y - circle.center.y) ** 2);
  if (c > circle.radius) {
    return false;
  }
  return true;
...
function isCreditCardNumber(ccn) {
  let numSet = ccn.toString().slice(0, -1).split('').reverse()
    .join('');
  for (let i = 0; i < numSet.length; i += 2) {
    let doubled = numSet[i] * 2;
    if (doubled >= 10) {
      doubled = 1 + (doubled % 10);
    }
    numSet = numSet.replace(numSet[i], doubled);
  }
  let sum = 0;
  ...
  const result = +`${ccn.toString().slice(0, -1)}${checkDigit}`;
  if (result === ccn) {
    return true;
  }
  return false;
